var chaty_settings = $style = chatycountry = "";
var isshowchaty = 0;
var isowner = 'no';
var mainDomain = "https://chaty-app.premio.io"; 
//"https://chaty-app.premio.io"
var chtbaseUrl = mainDomain;
...
function launch_chaty(){
    if (!document.querySelector(".chaty-widget").classList.contains("one_widget")) {
        document.querySelector(".chaty-widget").classList.add("chaty-widget-show");
    }
}
(function(){
...
var chatyappsJavaScript = function(jQuery){

    $style = "";
    $style += ".chaty-widget-is a{display: block; margin:0; padding:0; }";
    $style += ".chaty-widget-is svg{margin:0; padding:0;}";
    $style += ".chaty-main-widget { display: none; }";
    $style += ".chaty-in-desktop .chaty-main-widget.is-in-desktop { display: block; }";
    $style += ".chaty-in-mobile .chaty-main-widget.is-in-mobile { display: block; }";
    $style += ".chaty-widget.hide-widget { display: none !important; }";

    jQuery("body").append('<style>'+$style+'</style>');
    jQuery('head').append('<link rel="stylesheet" href="'+chtbaseUrl+'/assets/css/chaty-pro/chaty-front.css" type="text/css" media="all">');
    ...
    var $style = '';
    var url = chtbaseUrl + '/chaty_front';

    var chatyisowner = jQuery('#admin-bar-iframe').html();
    if(chatyisowner !== undefined){
        isowner = 'yes';
        createCookie('chatyisowner', 'yes', 1000);
    }
    var chatyisownerCookie = readCookie('chatyisowner');
	...
	var traffic_source = readCookie('chaty_traffic_source');
	
    if(isbotuser == 'no'){
        jQuery.ajax({
            type: "POST",
            url: url,
            data: {'shop_url': shopurl, 'isMobile' : isMobile, 'pathName' : pathName, 'isowner':isowner,'origin_landing_page': origin_landing_page,'HTTP_REFERER': myReferer,'traffic_source': traffic_source},
            //dataType: "JSON",
            success: function (response) {
				if(response != 'null' && response != ""){

                    var allwidgets = jQuery.parseJSON(response);
                    chaty_settings = allwidgets;
					//console.log("chaty_settings==>"+chaty_settings);
					chatyhard(jQuery);
                }
            }
        });
        ...
        !function (t) {
            ...
            function check_for_widget_data(index) {
                ...
                if(index < e.chaty_widgets.length) {
					if (  e.chaty_widgets[index].is_traffic_source !='' && traffic_source == null ) {
						createCookie('chaty_traffic_source', e.chaty_widgets[index].is_traffic_source, 1);
					}
				
				
                    if(e.chaty_widgets[index].isremovectacookie == 1){
                        createCookie("chaty_settings", "", "0");
                    }
                    ...
                    if(e.chaty_widgets[index].maxvisitor){
                        jQuery('head').append('<link rel="stylesheet" id="custom-google-fonts-css" href="https://fonts.googleapis.com/css?family=Poppins:400,600,700" type="text/css" media="all">');
                        jQuery('body').append(e.chaty_widgets[index].maxvisitormessage);
                    }
                }else {
					...
					if(jQuery(".chaty-main-widget.whatsapp-action-btn a").length) {
						jQuery(".chaty-main-widget.whatsapp-action-btn a").each(function(){
							...
							thisHref = thisHref.replace(/{title}/g, jQuery.trim(jQuery("head title").text()));
							thisHref = thisHref.replace(/{url}/g, encodeURI(window.location.href));
							jQuery(this).prop("href", thisHref);
						})
					}
            ...
            if(e.chaty_widgets.length > 0) {
                ...
                setTimeout(function(){
                    ...
                    var updatechatycounter = readCookie('updatechatycounter');
                    if(isowner == 'no' && isshowchaty == 1 && updatechatycounter != 'yes' && isBoatUser != true){
                        updatechtvisitor();
                    }
                },100);
            }
            ...
            function load_chaty_functions() {
                ...
                if(!chatyFunctionLoaded) {
                    ...
                    jQuery(document).on("click", ".i-trigger .i-trigger-open" , function(){
                        ...
                        updatechtwidgetclick(thisIndex);
                    });
					...
					jQuery(document).on("click", ".has-custom-chaty-popup.whatsapp-button", function(e){
						...
						if(jQuery(this).attr("data-popup") != undefined && jQuery(this).attr("data-popup") != "") {
							...
							if(jQuery(".chaty-inline-popup .default-value").length) {
								...
								jQuery(".chaty-whatsapp-msg").val(thisHref);
								jQuery(".chaty-whatsapp-phone").val(jQuery(".chaty-inline-popup .default-msg-phone").text());
								if ( jQuery(".chaty-inline-popup .default-msg-value").html() != '' ) {
									jQuery(".chaty-whatsapp-message").html(jQuery(".chaty-inline-popup .default-msg-value").html());
								} else {
									jQuery(".chaty-whatsapp-message").hide();
								}
							}
							jQuery("#chaty-widget-"+thisIndex).addClass("active");
							setTimeout(function(){
					...
					jQuery(document).on("submit", ".chaty-contact-form-data", function(e){
						...
						if(inputErrorCounter == 0) {
							...
							jQuery.ajax({
								url: url = chtbaseUrl + '/chaty_front/chaty_front_form_save_data',
								data: {
									...
									shop_id: $form.find(".chaty-field-shop").length?$form.find(".chaty-field-shop").val():"",
									setting_id: $form.find(".chaty-field-setting").length?$form.find(".chaty-field-setting").val():"",
								...
								success: function (response) {
									...
									if(response.status == 1) {
										...
										jQuery(".chaty-field-name, .chaty-field-email, .chaty-field-message, .chaty-field-phone").val("");
            ...
            function set_chaty_channels(thisIndex) {
                ...
                if(activeWidget == 0) {
                    ...
                    console.log('open chaty one widget');
					...
					console.log(chaty_settings.chaty_widgets[thisIndex]['rgb_color']);
					setCSSKeyFrames(chaty_settings.chaty_widgets[thisIndex]['rgb_color']);
				}
				
				jQuery("#chaty-widget-"+thisIndex).find(".i-trigger .chaty-widget-i-title p").css('color',chaty_settings.chaty_widgets[thisIndex]['cht_cta_text_color']);
				jQuery("#chaty-widget-"+thisIndex).find(".chaty-widget-i-title,.chaty-widget-i-title p").css('color',chaty_settings.chaty_widgets[thisIndex]['cht_cta_text_color']);
				jQuery("#chaty-widget-"+thisIndex).find(".chaty-widget-i-title").css('background-color',chaty_settings.chaty_widgets[thisIndex]['cht_cta_bg_color']);				

                jQuery("#chaty-widget-"+thisIndex+" .i-trigger svg, #chaty-widget-"+thisIndex+" .i-trigger img").wrap(function() {
                    //return "<div id='chaty-animation-"+ thisIndex +"' class='animation-svg' " + bgColor + "></div>";
            ...
            function check_for_time(index) {
                ...
                if (parseInt(chaty_settings.chaty_widgets[index].display_conditions) == 1) {
                    ...
                    if (displayRules.length > 0) {
						...
						var offset = chaty_settings.chaty_widgets[index].gmt.replace('UTC', ''); 						
						if( jQuery.isNumeric( offset )) {
							...
							//localDate.setHours( parseInt(localhours) + parseFloat(chaty_settings.chaty_widgets[index].gmt));
							
						} else {
							var localDate = new Date().toLocaleString('en-US', { timeZone: chaty_settings.chaty_widgets[index].gmt });
							localDate = new Date(localDate);
						}
						//localDate.setHours(localDate.getHours() + (chaty_settings.chaty_widgets[index].gmt));
                        ...
                        for (var rule = 0; rule < displayRules.length; rule++) {
                            ...
                            if (checkForTime == 1) {
                                if (utcHours > displayRules[rule].start_hours && utcHours < displayRules[rule].end_hours) {
                                    ...
                                    console.log('utcHours'+utcHours+' '+displayRules[rule].start_hours+' end:'+displayRules[rule].end_hours);
                                    console.log('utcMin'+utcMin+' '+displayRules[rule].start_min+' end:'+displayRules[rule].end_min);
                                    if(utcHours == displayRules[rule].start_hours){
                                        if (utcMin >= displayRules[rule].start_min) {
                                            hourStatus = 1;
                                        }
                                    }else if(utcHours > displayRules[rule].start_hours){
                                        if (utcMin >= displayRules[rule].start_min) {
                                            hourStatus = 1;
                                        }
                                    }
                                    ...
                                    if(nextday == 1 && utcHours <= displayRules[rule].end_hours){
                                        if(utcHours < displayRules[rule].end_hours){
                                            hourStatus = 1;
                                        }else if(utcHours == displayRules[rule].end_hours){
                                            if (utcMin <= displayRules[rule].end_min) {
                                                hourStatus = 1;
                                            }
                                        }
            ...
            function set_chaty_widget(widgetIndex) {
                ...
                "" != i("display_cta"), token = "", jQuery(document).ready(function () {
                    "true" == chaty_settings.chaty_widgets[widgetIndex].active && (function (e, n) {
                        updatechtwidgetvisitor(widgetIndex);
                        isshowchaty = 1;
                            ...
                            return t.each(chaty_settings.chaty_widgets[widgetIndex].social, function (t, o) {
								...
								if(parseInt(chaty_settings.chaty_widgets[widgetIndex].social[t].has_custom_popup) == 1) {
									...
									if(chaty_settings.chaty_widgets[widgetIndex].social[t].channel_type == "whatsapp") {
										...
										targetAction = "_blank";
										if (jQuery("body").hasClass("chaty-in-mobile")) {
											var whatsappVal = chaty_settings.chaty_widgets[widgetIndex].social[t].href_url;
											if( whatsappVal=='javascript:;' ){
												var whatsapp_url = 'https://wa.me/'+chaty_settings.chaty_widgets[widgetIndex].social[t].val+'?text='+chaty_settings.chaty_widgets[widgetIndex].social[t].pre_set_message;
												chaty_settings.chaty_widgets[widgetIndex].social[t].href_url = whatsapp_url; 
												targetAction = "";
											}
										}
										desktop_target = "";
                                ...
                                socialString = '<div id="'+chaty_settings.chaty_widgets[widgetIndex].social[t].channel_id+'" data-rgb="'+chaty_settings.chaty_widgets[widgetIndex].social[t].rbg_color+'" class="chaty-widget-i chaty-main-widget ' + desktopClass + " " + mobileClass + " " + extra_class + " channel-" + chaty_settings.chaty_widgets[widgetIndex].social[t].social_channel + '"  data-popup="'+chaty_settings.chaty_widgets[widgetIndex].social[t].popup_html+'" data-title="' + chaty_settings.chaty_widgets[widgetIndex].social[t].val + '" data-nonce="' + chaty_settings.chaty_widgets[widgetIndex].social[t].channel_nonce + '" id="chaty-channel-' + chaty_settings.chaty_widgets[widgetIndex].social[t].social_channel + '" data-channel="' + chaty_settings.chaty_widgets[widgetIndex].social[t].social_channel + '" data-code="' + chaty_settings.chaty_widgets[widgetIndex].social[t].qr_code_image + '">', bgColor = "", "" != chaty_settings.chaty_widgets[widgetIndex].social[t].bg_color && (socialString += "<style>."+chaty_settings.chaty_widgets[widgetIndex].social[t].social_channel+"-"+widgetIndex+"-channel .color-element {fill: " + chaty_settings.chaty_widgets[widgetIndex].social[t].bg_color + "; background:" + chaty_settings.chaty_widgets[widgetIndex].social[t].bg_color + ";}#chaty-widget-"+widgetIndex+".chaty-widget .chaty-widget-i:not(.no-tooltip):before{border-color: transparent transparent transparent "+chaty_settings.chaty_widgets[widgetIndex].cht_cta_bg_color +";}#chaty-widget-"+widgetIndex+".chaty-widget.chaty-widget-is-right .chaty-widget-i:before{ border-color: transparent "+chaty_settings.chaty_widgets[widgetIndex].cht_cta_bg_color +" transparent transparent}</style>", bgColor = "style='background-color: " + chaty_settings.chaty_widgets[widgetIndex].social[t].bg_color + ";'"), socialString += "<a class='set-url-target' "+onclick_settings+" rel='noopener' data-mobile-target='" + chaty_settings.chaty_widgets[widgetIndex].social[t].mobile_target + "' data-desktop-target='" + chaty_settings.chaty_widgets[widgetIndex].social[t].desktop_target + "' target='" + targetAction + "' href='" + chaty_settings.chaty_widgets[widgetIndex].social[t].href_url + "' >", "" != chaty_settings.chaty_widgets[widgetIndex].social[t].img_url ? socialString += "<span class='chaty-social-img'><img src='" + chaty_settings.chaty_widgets[widgetIndex].social[t].img_url + "' alt='" + chaty_settings.chaty_widgets[widgetIndex].social[t].title + "' /></span>" : socialString += chaty_settings.chaty_widgets[widgetIndex].social[t].default_icon, socialString += "</a>", socialString += "<div class='chaty-widget-i-title'><p>" + chaty_settings.chaty_widgets[widgetIndex].social[t].title + "</p></div>", socialString += "</div>";
                                    i += socialString;
                                }), i
            ...
            function chtownerevent(event) {
                jQuery.ajax({
                    type: "POST",
                    url: chtbaseUrl+'/chaty_front/addmaxvisitors',
                    data: {'storeid': chaty_settings.chaty_widgets[0].storeid, 'event' : event},
                    //dataType: "JSON",
                    success: function (response) {
            ...
            function updatechtvisitor(){
                createCookie('updatechatycounter', 'yes', 1);
                url = chtbaseUrl + '/chaty_front/updateVistor';
                jQuery.ajax({
                    type: "POST",
                    url: url,
                ...
                if(shopurl == 'micishibi.myshopify.com'){
                    urllog = chtbaseUrl + '/chaty_front/addVistorlog';
                    var $ipurl = 'https://www.cloudflare.com/cdn-cgi/trace';
                    jQuery.get($ipurl, function (cloudflaredata) {
                        chtip = cloudflaredata.match("ip=(.*)");
                        chtuag = cloudflaredata.match("uag=(.*)");
                        chtloc = cloudflaredata.match("loc=(.*)");
                        chtlogdata = chtip+'&&&'+chtuag+'&&&'+chtloc;
                        jQuery.ajax({
                                type: "POST",
                                url: urllog,
                                data: {'shop_url': shopurl, 'chtlogdata':chtlogdata},
                                //dataType: "JSON",
                                success: function (response) {
                                }
                        });
            ...
            function updatechtwidgetvisitor(widgetindex){
                var updatechtwidgetvisitor = readCookie('updatechtwidgetvisitor_'+widgetindex);
                if(updatechtwidgetvisitor != 'yes'){
                    var chtwidid = chaty_settings.chaty_widgets[widgetindex].widget_index;
                    createCookie('updatechtwidgetvisitor_'+widgetindex, 'yes', 1);
                    url = chtbaseUrl + '/chaty_front/updatechtwidgetvisitor';
                    jQuery.ajax({
                        type: "POST",
                        url: url,
                        data: {'shop_url': shopurl, 'widgetid':chtwidid, 'type':'visitorcount'},
                        success: function (response) {
            ...
            function updatechtwidgetclick(widgetindex){
                var updatechtwidgetclick = readCookie('updatechtwidgetclick_'+widgetindex);
                if(updatechtwidgetclick != 'yes'){
                    var chtwidid = chaty_settings.chaty_widgets[widgetindex].widget_index;
                    createCookie('updatechtwidgetclick_'+widgetindex, 'yes', 1);
                    url = chtbaseUrl + '/chaty_front/updatechtwidgetclick';
                    jQuery.ajax({
                        type: "POST",
                        url: url,
                        data: {'shop_url': shopurl, 'widgetid':chtwidid, 'type':'visitorclick'},
                        success: function (response) {
            ...
            jQuery("body").on("click", ".chtownerpopupclose", function () {
                if(jQuery(this).hasClass('riskabovefourdays') && !t(this).hasClass('chteventsopen')){
                    //first click_on_x
                    jQuery(this).addClass('chteventsopen');
                    jQuery("body .chtownerpopup .chtcontent, body .chtownerpopup .chtfooter").hide();
                    jQuery("body .chtownerpopup .chtcontent.reskmoreevents").show();
                }else if(t(this).hasClass('chteventsopen')){
                    chtownerevent('dont_remind_me');
                    jQuery("body #chatyownerpopup").hide();
                }else{
                    //second time click_on_x
                    jQuery("body #chatyownerpopup").hide();
                    chtownerevent('click_on_x');
                }
            });
            jQuery("body").on("click", ".chatyownerremind .chtremind", function () {
                chtownerevent('clicked_on_remind_me');
                jQuery("body #chatyownerpopup").hide();
            });
            jQuery("body").on("click", ".chatyownerremind .chtdontremind", function () {
                chtownerevent('dont_remind_me');
                jQuery("body #chatyownerpopup").hide();
            });
            jQuery("body").on("click", ".chtupgradenow", function () {
                chtownerevent('click_on_upgrade');
                jQuery("body #chatyownerpopup").hide();
            });
            setTimeout(function(){
                jQuery('body #chatyownerpopup .chtownerchannels').html(jQuery('body .chaty-widget'));
            }, 500);
        }(jQuery);
    }, 12: function (t, e) {
...
if ((typeof jQuery === 'undefined') || (parseFloat(jQuery.fn.jquery) < 1.7)) {
	loadScript('//ajax.googleapis.com/ajax/libs/jquery/3.3.1/jquery.min.js', function(){
      ...
      CHATYAPPS = jQuery.noConflict(true);
	  CHATYAPPS(document).ready(function() {
		chatyappsJavaScript(CHATYAPPS);
	  });
	});
 } else {
	chatyappsJavaScript(jQuery);
  }
})();
...
function close_chaty() {
	...
	loadScript_widget('//ajax.googleapis.com/ajax/libs/jquery/3.3.1/jquery.min.js', function(){
		CHATYAPPS = jQuery.noConflict(true);
		CHATYAPPS('.chaty-widget').removeClass('couonapp-active');
		CHATYAPPS('.chaty-widget').each(function(){
			CHATYAPPS(this).removeClass("chaty-widget-show").addClass("none-widget-show");
		});		
	});
}